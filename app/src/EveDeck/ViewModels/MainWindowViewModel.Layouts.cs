...
public sealed partial class MainWindowViewModel
{
    ...
    // Rename a custom profile in place. Built-in presets keep their names (they are regenerated by
    // PresetFactory and the user can always Duplicate one to get a renamable copy), so the command is
    // gated on IsBuiltIn rather than silently no-oping here.
    private void RenameProfile()
    {
        if (SelectedProfile is null) return;
        if (SelectedProfile.IsBuiltIn) { Log.Warn("Built-in presets cannot be renamed. Duplicate one to make a custom profile."); return; }

        var profile = SelectedProfile;
        var dialog = new Views.RenameProfileDialog(profile.Name)
        {
            Owner = System.Windows.Application.Current.MainWindow,
        };
        if (dialog.ShowDialog() != true) return;
        ...
        // Pass the profile itself as `ignore` so re-confirming the same name (or only changing its
        // case) isn't treated as a clash and bumped to "Name 2".
        var newName = UniqueProfileName(dialog.ProfileName, profile);
        if (newName.Equals(oldName, StringComparison.Ordinal)) return;

        profile.Name = newName;

        // LayoutProfile is a plain POCO, so nothing notifies the list. Refresh re-reads the names AND
        // re-applies the Name sort, then re-point the selection since Refresh resets the view's cursor.
        ProfilesView.Refresh();
        SelectedProfile = profile;
        OnPropertyChanged(nameof(SelectedProfile));
        RebuildLayoutPreview();
        Save();
        Log.Info($"Renamed profile '{oldName}' to '{newName}'.");
    }
    ...
    // ignore: a profile that should not count as a clash -- used by rename so a profile never collides
    // with its own current name.
    private string UniqueProfileName(string baseName, LayoutProfile? ignore = null)
    {
        var name = baseName;
        var i = 2;
        while (Profiles.Any(p => !ReferenceEquals(p, ignore) && p.Name.Equals(name, StringComparison.OrdinalIgnoreCase)))
            name = $"{baseName} {i++}";
        return name;
    }
